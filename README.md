...
```
  ...
  Example — two editors + a button that reads both contents (based on the sample app):
...
Security reminder: always sanitize editor HTML on the server before storing or rendering. I have not included this to keep the project as simple as possible, but feel free to add  this functionality using a trusted library as for example DOMPurify.
